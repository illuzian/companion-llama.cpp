    ...
    std::vector<raw_buffer> & out_files)
{
    ...
    // Reasoning budget: pass parameters through to sampling layer
    {
        ...
        if (!chat_params.thinking_end_tags.empty()) {
            ...
            llama_params["reasoning_transition_before_cue"] =
                json_value(body, "reasoning_transition_before_cue", std::string());
            llama_params["reasoning_transition_after_cue"] =
                json_value(body, "reasoning_transition_after_cue", std::string());
        }
    }
